...
program define balance_table
	...
	syntax varlist [if] using/, Treatment(varlist) [Cluster(varlist)] ///
		...
		[title(passthru)] [marker(passthru)]
	...
	**Export table
	...
	if regexm("`using'",".tex$")==1 {
		...
		n di "marker = `marker'"
		tempfile temp
		qui texsave using "`temp'", location(hp) autonumber varlabels replace frag  size(3) `marker' `title' footnote("Standard Deviations in parantheses; *p $<$ 0.1,**p $<$ 0.05,***p $<$ 0.01")
